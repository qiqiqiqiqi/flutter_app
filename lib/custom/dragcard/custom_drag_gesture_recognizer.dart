...
class CustomPanGestureReconizer extends DragGestureRecognizer {
  /// Create a gesture recognizer for tracking movement on a plane.
  Direction direction;

  CustomPanGestureReconizer({Object debugOwner, this.direction})
      : super(debugOwner: debugOwner);

  @override
  Offset _getDeltaForDetails(Offset delta) {
    if (!direction.hasHorizontal()) {
  ...
  bool get _hasSufficientPendingDragDeltaToAccept {
    ...
    print("_hasSufficientPendingDragDeltaToAccept:result=$result");
    return result;
  }
...
class Direction {
  static const int direction_left = 1 << 0;
  static const int direction_top = 1 << 1;
  static const int direction_right = 1 << 2;
  static const int direction_bottom = 1 << 3;
  static const int direction_all =
      direction_left | direction_top | direction_right | direction_bottom;
  final int direction;

  const Direction({this.direction});

  static const Direction left = Direction(direction: direction_left);
  static const Direction top = Direction(direction: direction_top);
  static const Direction right = Direction(direction: direction_right);
  static const Direction bottom = Direction(direction: direction_bottom);
  static const Direction all = Direction(direction: direction_all);

  Direction operator |(Direction direction) {
    return new Direction(direction: this.direction | direction.direction);
  }

  bool hasLeft() {
    return (direction & direction_left) != 0;
  }

  bool hasTop() {
    return (direction & direction_top) != 0;
  }

  bool hasRight() {
    return (direction & direction_right) != 0;
  }

  bool hasBottom() {
    return (direction & direction_bottom) != 0;
  }
