...
class DrawImageUtils {
  ...
  static Future<flutterUi.Image> drawMaskImage(String originImageUrl,
    ...
    Future.wait([
      ...
      //合成
      canvas.drawImage(result[1], Offset(0, 0) /*offset*/, paint);
      canvas.drawImageRect(
          result[0],
          Rect.fromLTWH(
  ...
  ///  合并
  static Future<flutterUi.Image> drawMergeImage(
      {String bgImageUrl, String mergeImageUrl, Offset offset}) {
    Completer<flutterUi.Image> completer = new Completer<flutterUi.Image>();
    Future.wait([
      //获取裁剪图片
      ImageLoader.load(mergeImageUrl),
      ImageLoader.loadNet(bgImageUrl),
    ]).then((result) {
      Paint paint = new Paint();
      PictureRecorder recorder = PictureRecorder();
      Canvas canvas = Canvas(recorder);
      ...
      Rect dst = Rect.fromLTWH(
          0, 0, result[0].width.toDouble(), result[0].height.toDouble());
      canvas.drawImageRect(result[0], src, dst, paint);
